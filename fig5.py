...
for i in tqdm(range(len(simclr_correct_a))):
    if daco_correct_a[i] and not npid_correct_a[i] and not simclr_correct_a[i] and not moco_correct_a[i]:
...
for i in tqdm(range(len(simclr_correct_b))):
    if daco_correct_b[i] and not npid_correct_b[i] and not simclr_correct_b[i] and not moco_correct_b[i]:
        ...
        Image.open(npid_rank_path).save(
            '{}/npid_{:.2f}.png'.format(save_path, npid_sim_b[i][npid_idx_b[i][0]].item()))
        simclr_rank_path = val_data.original_images[simclr_idx_b[i][0]]
        Image.open(simclr_rank_path).save(
            '{}/simclr_{:.2f}.png'.format(save_path, simclr_sim_b[i][simclr_idx_b[i][0]].item()))
        moco_rank_path = val_data.original_images[moco_idx_b[i][0]]
        Image.open(moco_rank_path).save(
            '{}/moco_{:.2f}.png'.format(save_path, moco_sim_b[i][moco_idx_b[i][0]].item()))
        daco_rank_path = val_data.original_images[daco_idx_b[i][0]]
        Image.open(daco_rank_path).save(
            '{}/daco_{:.2f}.png'.format(save_path, daco_sim_b[i][daco_idx_b[i][0]].item()))

for i in tqdm(range(len(simclr_correct))):
    if daco_correct[i] and not npid_correct[i] and not simclr_correct[i] and not moco_correct[i]:
        query_path = val_data.original_images[i]
        ...
        Image.open(npid_rank_path).save(
            '{}/npid_{:.2f}.png'.format(save_path, npid_sim[i][npid_idx[i][0]].item()))
        simclr_rank_path = val_data.original_images[simclr_idx[i][0]]
        Image.open(simclr_rank_path).save(
            '{}/simclr_{:.2f}.png'.format(save_path, simclr_sim[i][simclr_idx[i][0]].item()))
        moco_rank_path = val_data.original_images[moco_idx[i][0]]
        Image.open(moco_rank_path).save(
            '{}/moco_{:.2f}.png'.format(save_path, moco_sim[i][moco_idx[i][0]].item()))
        daco_rank_path = val_data.original_images[daco_idx[i][0]]
        Image.open(daco_rank_path).save(
            '{}/daco_{:.2f}.png'.format(save_path, daco_sim[i][daco_idx[i][0]].item()))
